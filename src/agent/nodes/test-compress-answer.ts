...
async function run(repoKey: string, query: string, hint: string | null) {
  ...
  const { answer, citations } = await generateAnswer(query, compressedContext);
  console.log(`\nAnswer:\n${answer}`);
  console.log(`\nCitations (${citations.length}):`, citations.map((c) => c.symbolId));
}

async function main() {
  await run("local:D:\\project_og\\sample-repo", "What breaks if I change loadConfig?", "loadConfig");
  await run(
